...
func (m *mapTransformation2) Process(chunk table.Chunk, d *execute.TransportDataset, mem memory.Allocator) error {
	...
	// Execute function.
	cols, arrs, err := m.execute(chunk, fn, mem)
	if err != nil {
		return err
	}
...
// initialize will initialize the builders using the column metadata.
func (m *mapTransformation2) initialize(cols []flux.ColMeta, mem memory.Allocator) []array.Builder {
...
func (m *mapTransformation2) createSchema(fn *execute.RowMapPreparedFn, record values.Object) ([]flux.ColMeta, error) {
	returnType := fn.Type()
...
// execute will execute the function on each row of the table chunk.
...
func (m *mapTransformation2) Dispose() {}
