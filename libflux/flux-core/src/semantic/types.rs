...
use crate::{
    ...
    semantic::{
        ...
        nodes::Symbol,
        sub::{
            apply2, apply3, apply4, merge, merge4, merge_collect, Substitutable, Substituter,
            Substitution,
        },
    },
};
...
impl MonoType {
    ...
    /// Visits all `MonoTypes` in `self` and calls `visit_type` on them, replacing them if the
    /// visitor returns `Some`
    pub fn visit(&self, visitor: &mut impl TypeVisitor) -> Option<MonoType> {
        match visitor.visit_type(self) {
            Some(typ) => Some(typ.walk(visitor).unwrap_or(typ)),
            None => self.walk(visitor),
        }
    }
    ...
    fn walk(&self, visitor: &mut impl TypeVisitor) -> Option<MonoType> {
        match self {
            ...
            MonoType::Dict(dict) => merge(
                ...
                &dict.val,
                dict.val.visit(visitor),
            )
            .map(|(key, val)| MonoType::from(Dictionary { key, val })),
            MonoType::Record(obj) => obj.visit(visitor).map(MonoType::record),
            MonoType::Fun(fun) => merge4(
                &fun.req,
                merge_collect(
                    visitor,
                    &fun.req,
                    |visitor, (k, v)| v.visit(visitor).map(|v| (k.clone(), v)),
                ...
                &fun.pipe,
                match &fun.pipe {
                    Some(pipe) => pipe.visit(visitor).map(Some),
                    None => None,
                },
                &fun.retn,
                fun.retn.visit(visitor),
            )
            .map(|(req, opt, pipe, retn)| {
...
impl Record {
    ...
    fn visit(&self, visitor: &mut impl TypeVisitor) -> Option<Self> {
        match self {
            Record::Empty => None,
            Record::Extension { head, tail } => {
                merge(head, head.visit(visitor), tail, tail.visit(visitor))
                    .map(|(head, tail)| Record::Extension { head, tail })
            }
        }
    }
}
...
{
    fn visit(&self, visitor: &mut impl TypeVisitor) -> Option<Self> {
        self.v.visit(visitor).map(|v| Property {
            k: self.k.clone(),
            v,
        })
    }
...
pub trait TypeVisitor {
    /// Replaces `typ` with the returned type when `Some` is returned, otherwise leaves the type
    /// unmodified
...
where
    F: FnMut(&MonoType) -> Option<MonoType>,
{
    fn visit_type(&mut self, typ: &MonoType) -> Option<MonoType> {
        self(typ)
    }
...
/// Replaces all type variables with their actual type and labels with string
pub fn finalize_types(sub: &mut Substitution, typ: &MonoType) -> Option<MonoType> {
    typ.visit(&mut |typ: &MonoType| match typ {
